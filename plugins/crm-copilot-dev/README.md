# Beaconfireinc CRM Copilot V3 (Dev) Plugin
...
The configured development endpoint is:

```text
https://api-crm-mcp-dev.beaconfireinc.com/mcp
```

After installing the plugin, open Claude Code's MCP panel, select `crm-copilot-dev`, and choose **Authenticate**. Complete the company AuthSome OAuth flow in the browser. Claude Code manages the OAuth tokens; never place client secrets, access tokens, or Authorization headers in this repository.
...
```bash
claude --plugin-dir ./plugins/crm-copilot-dev
```
...
/plugin install crm-copilot-dev@beaconfireinc
```

Restart Claude Code or reload plugins after installation.

The bundled `crm-copilot-v3-dev` skill is the authoritative conversation and tool-routing contract for this MCP service.
