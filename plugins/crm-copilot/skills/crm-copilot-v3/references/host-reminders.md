...
1. Use the host's native notification, reminder, calendar, automation, or
   scheduled-task capability when one is available. Create a one-time
   notification at `hostNotification.scheduleAt` (the confirmed `remind_at`
   time), preserving the confirmed IANA timezone. On mobile this must be a
   host-level notification/calendar reminder, not merely a CRM database row or
   an MCP App page.
...
4. If the host cannot schedule notifications, report the CRM reminder as
   ...
   database row itself will notify Claude. Do not report the overall reminder
   workflow as complete until the host call returns success.
